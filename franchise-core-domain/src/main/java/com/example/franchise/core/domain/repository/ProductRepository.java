...
public interface ProductRepository extends JpaRepository<ProductRecord, Long> {
    ...
    @Query("SELECT new com.example.franchise.core.domain.dto.ProductTopDto(p.id, p.name, p.stock_quantity, b.id, b.name) " +
            "FROM FranchiseRecord f " +
            "JOIN f.branches b " +
            "JOIN b.products p " +
            "WHERE f.id = :franchiseId AND p.stock_quantity = (SELECT MAX(p2.stock_quantity) FROM ProductRecord p2 WHERE p2.branch.id = b.id)")
